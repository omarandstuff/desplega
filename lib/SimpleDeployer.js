...
SimpleDeployer.prototype.deploy = function () {
  connect.apply(this)
}
...
function initLifeCycle () {
  ...
  executeRemoteCommand.apply(this, ['git clone https://github.com/omarandstuff/mdomotk'])
}

function executeRemoteCommand (command) {
  const commandExcevutionInitialTime = process.hrtime()
  ...
  this.connection.exec(command, function (error, stream) {
    ...
    stream.on('close', function (code, signal) {
      const timeToHere = process.hrtime(commandExcevutionInitialTime)
      // Here we have all the stdout data and code to use letter
    })
