...
function continueLifeCycle(lastResult, lastCode, excecutionTime, command) {
  const currentCommand = command || selectCommand.apply(this)

  if (currentCommand) {
    currentCommand.options = currentCommand.options || []

    if (currentCommand.dynamic) {
      var rendered = currentCommand.dynamic(lastResult, lastCode, excecutionTime)

      currentCommand.command = rendered.command
      currentCommand.options = rendered.options || []
    }
...
function showResults(command, lastResult, lastCode, excecutionTime, error) {
  if(lastCode !== 0 && !command.continueOnErrorCode) {
    ...
    if(this.configuration.onFail) {
      const onFailGeneratedCommand = this.configuration.onFail(command, lastResult, lastCode, excecutionTime, error)

      onFailGeneratedCommand.failCommand = true

      continueLifeCycle.apply(this, [lastResult, lastCode, excecutionTime, onFailGeneratedCommand])
    } else {
      this.connection.end()
    }
  } else {
    ...
    if(command.local) {
      ...
      console.log(chalk.green('    ✔ ' + formatUnits(this.commanInBlock) + ' ' + this.configuration.username + '@' + this.configuration.host), formatTime('ss.SSS', excecutionTime) + 's')
    }

    if(!command.failCommand) {
      continueLifeCycle.apply(this, [lastResult, lastCode, excecutionTime])
    }
  }
}
...
function excecuteLocalCommand (command) {
  ...
  child_process.exec(command.command + ' ' + command.options.join(' '), { maxBuffer: 512 * 1024 * 1024 }, function (error, stdout, stderr) {
    const timeToHere = process.hrtime(commandExcecutionInitialTime)

    showResults.apply(self, [command, stdout, (error || {}).code || 0, timeToHere, stderr])
  })
}
...
function connect () {
  ...
  this.connection.connect({
    ...
    port: this.configuration.port || 22,
    username: this.configuration.username,
    privateKey: getPrivateKey(this.configuration.privateKeyPath),
