...
function showResults(command, lastResult, lastCode, excecutionTime) {
...
function excecuteLocalCommand (command) {
  ...
  excecution.on('close', (code) => {
    ...
    showResults.apply(this, [command, fullOutput, code, timeToHere])
  });

  excecution.on('error', (error) => {
    const timeToHere = process.hrtime(commandExcecutionInitialTime)

    showResults.apply(this, [command, fullOutput, error, timeToHere])
  });
